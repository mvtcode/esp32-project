...
foreach ($f in $files) {
    ...
    # 3. Kiem tra Codec Format (Phat hien file AAC/M4A fake duoi .mp3)
    $codecName = & $ffprobe -v error -select_streams a:0 -show_entries stream=codec_name -of csv=p=0 $filePath 2>$null
    $isFakeMp3 = ($codecName -ne "mp3")

    # 4. Kiem tra sample rate
    ...
    $hasMissingBitrate = (!$bitrateStr -or $bitrateStr -eq "N/A" -or $bitrateStr -eq "0")
    ...
    $needFix = $hasStreamError -or $hasCoverArt -or $isFakeMp3 -or $needResample -or $hasMissingBitrate

    if ($needFix) {
        $reasons = @()
        if ($isFakeMp3)          { $reasons += "Dinh dang goc la $codecName (khong phai MP3 that)" }
        if ($hasStreamError)     { $reasons += "Loi frame audio" }
        if ($hasCoverArt)        { $reasons += "Co anh bia (Cover Art)" }
        if ($needResample)       { $reasons += "Tan so $sampleRate Hz (can 44.1kHz)" }
        if ($hasMissingBitrate)  { $reasons += "Thieu/Loi Bitrate Header" }
        
        Write-Host "   -> [CAN FIX] Ly do: $($reasons -join ', ')" -ForegroundColor Yellow
        Write-Host "   -> Dang convert sang MP3 chuẩn (libmp3lame, CBR ${bitrateKbps} kbps, 44.1kHz)..." -ForegroundColor Gray

        & $ffmpeg -y -i $filePath -vn -map_metadata -1 -ar 44100 -ac 2 -b:a "${bitrateKbps}k" -c:a libmp3lame -write_xing 1 $outPath 2>$null
