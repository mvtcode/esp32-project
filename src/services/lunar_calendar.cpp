...
static const char* CAN[] = {"Giáp", "Ất", "Bính", "Đinh", "Mậu", "Kỷ", "Canh", "Tân", "Nhâm", "Quý"};
static const char* CHI[] = {"Tý", "Sửu", "Dần", "Mão", "Thìn", "Tỵ", "Ngọ", "Mùi", "Thân", "Dậu", "Tuất", "Hợi"};
...
static int getNewMoonDay(int k) {
    ...
    float M  = 359.2242f + 29.10535608f * k;
    float Mp = 306.0253f + 385.81691806f * k;
    float F  = 21.2964f  + 390.67050646f * k;

    float C =
        (0.1734f - 0.000393f * T) * sinf(M * DR)
        - 0.4068f * sinf(Mp * DR)
        + 0.0161f * sinf(2 * Mp * DR)
        + 0.0104f * sinf(2 * F * DR);

    float deltaT = (T < -11)
        ? (0.001f + 0.000839f * T)
        : (-0.000278f + 0.000265f * T);
...
static int getLeapMonthOffset(int a11) {
    int k = (int)((a11 - 2415021) / 29.530588f);
    int last = -1;
    for (int i = 1; i < 14; i++) {
...
void solarToLunar(int solarYear, int solarMonth, int solarDay, 
    ...
    int k = (int)((jd - 2415021) / 29.530588f);
    int nm = getNewMoonDay(k);
...
const char* getLunarYearName(int lunarYear) {
    static char buf[32];
    int canIdx = ((lunarYear + 6) % 10 + 10) % 10;
    int chiIdx = ((lunarYear + 8) % 12 + 12) % 12;
    snprintf(buf, sizeof(buf), "%s %s", CAN[canIdx], CHI[chiIdx]);
    return buf;
}
...
const char* getHolidayName(int solarDay, int solarMonth, int lunarDay, int lunarMonth) {
    // 1. Ngày lễ Âm lịch
    if (lunarMonth == 1) {
        if (lunarDay == 1) return "Mùng 1 Tết Nguyên Đán";
        if (lunarDay == 2) return "Mùng 2 Tết Nguyên Đán";
        if (lunarDay == 3) return "Mùng 3 Tết Nguyên Đán";
        if (lunarDay == 15) return "Tết Nguyên Tiêu";
    } else if (lunarMonth == 3 && lunarDay == 10) {
        return "Giỗ Tổ Hùng Vương";
    } else if (lunarMonth == 4 && lunarDay == 15) {
        return "Lễ Phật Đản";
        ...
        if (lunarDay == 29 || lunarDay == 30) return "Đêm Giao Thừa";
    }

    // 2. Ngày lễ Dương lịch
    if (solarMonth == 1 && solarDay == 1) return "Tết Dương Lịch";
    if (solarMonth == 2 && solarDay == 14) return "Lễ Tình Nhân (Valentine)";
    if (solarMonth == 3 && solarDay == 8) return "Quốc Tế Phụ Nữ";
    if (solarMonth == 4 && solarDay == 30) return "Giải Phóng Miền Nam";
    if (solarMonth == 5 && solarDay == 1) return "Quốc Tế Lao Động";
    if (solarMonth == 6 && solarDay == 1) return "Quốc Tế Thiếu Nhi";
    if (solarMonth == 9 && solarDay == 2) return "Quốc Khánh Việt Nam";
    if (solarMonth == 10 && solarDay == 20) return "Phụ Nữ Việt Nam";
    if (solarMonth == 11 && solarDay == 20) return "Nhà Giáo Việt Nam";
    if (solarMonth == 12 && (solarDay == 24 || solarDay == 25)) return "Lễ Giáng Sinh";
