...
void WeatherService::init() {
    ...
    last_fetch_time = 0;
    is_fetching = false;
    LOG_I("Weather", "Initialized for City: %s (%.4f, %.4f)", city.c_str(), latitude, longitude);
}
...
void WeatherService::setLocation(const char *cityName, float lat, float lon) {
    ...
    LOG_I("Weather", "Location changed to: %s (%.4f, %.4f)", cityName, lat, lon);
}
...
void WeatherService::fetchWeatherTask(void *param) {
    {
        ...
        LOG_D("Weather", "Fetching Open-Meteo: %s", url);
        http.begin(url);
        ...
        if (httpCode == HTTP_CODE_OK) {
            ...
            if (curIdx != -1) {
                ...
                LOG_I("Weather", "-> %s: %.1f C (Feels: %.1f), Hum: %d%%, Code: %d (%s), Wind: %d km/h, UV: %d",
...
void WeatherService::update(bool wifiConnected, bool force) {
    ...
    if (!is_fetching && (force || last_fetch_time == 0 || (now - last_fetch_time >= intervalMs))) {
        ...
        if (res != pdPASS) {
            LOG_E("Weather", "Task creation failed");
            is_fetching = false;
        }
    }
}
