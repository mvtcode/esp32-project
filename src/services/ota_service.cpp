...
#if __has_include("audio_player_service.h")
...
static void releaseAudioIfPresent() {
#if __has_include("audio_player_service.h")
...
static void restoreAudioIfPresent() {
#if __has_include("audio_player_service.h")
    AudioPlayerService::init();
#endif
}

static void resetNvsDefaults() {
#if __has_include("config_manager.h")
...
static void saveChangelogAndReleaseDate(const String& changelog, const String& releaseDate) {
#if __has_include("config_manager.h")
    if (changelog.length() > 0) ConfigManager::setChangelog(changelog);
    if (releaseDate.length() > 0) ConfigManager::setReleaseDate(releaseDate);
    ConfigManager::flush();
#else
    Preferences prefs;
    if (prefs.begin("ota_cfg", false)) {
        if (changelog.length() > 0) prefs.putString("changelog", changelog);
        if (releaseDate.length() > 0) prefs.putString("release_date", releaseDate);
        prefs.end();
    }
#endif
}
...
bool OtaService::checkUpdate(OtaInfo& info) {
    ...
    if (info.version != currentVersion) {
        ...
        saveChangelogAndReleaseDate(info.changelog, info.releaseDate);
    }
...
bool OtaService::startUpdate(const String& firmwareUrl, bool clearNvs, const String& changelog, const String& releaseDate, OtaProgressCallback progressCb, OtaStatusCallback statusCb) {
    ...
    // Giải phóng hoàn toàn AudioPlayer (nếu có) để giải phóng >40KB DRAM cho HTTPS TLS & OTA buffer
    releaseAudioIfPresent();
...
void OtaService::otaTask(void* param) {
    ...
    saveChangelogAndReleaseDate(s_pendingChangelog, s_pendingReleaseDate);
