    ...
    if (status.mode == MODE_MP3 && status.isPlaying) {
      ...
      if (size > 0) {
        ...
        if (status.isPlaying)
          ...
          currentMs = status.lastPauseStart - status.trackStartTime -
                      status.trackPausedTime;

        // One-time Stable Total Time Calculation logic logic moved here or
        // simplified? The original logic had a static locked variable. Keeping
        // that logic here for now.
